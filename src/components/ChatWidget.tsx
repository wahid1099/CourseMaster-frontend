...
const ChatWidget: React.FC = () => {
  ...
  const fetchSupportAgent = async () => {
    try {
      ...
      const response = await axios.get('/api/chat/support-agents', { withCredentials: true });
  ...
  const fetchChatHistory = async () => {
    if (!supportId) return;
    try {
      const response = await axios.get(`/api/chat/history/${supportId}`, { withCredentials: true });
