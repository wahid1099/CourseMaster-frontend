...
const AdminDashboard: React.FC = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      const response = await axios.get(`${API_URL}/admin/analytics`, { withCredentials: true });
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="card">
            <div className="stat-card">
              <FiBook className="stat-icon" style={{ color: 'var(--primary)' }} />
              <div>
          ...
          <div className="card">
            <div className="stat-card">
              <FiUsers className="stat-icon" style={{ color: 'var(--success)' }} />
              <div>
          ...
          <div className="card">
            <div className="stat-card">
              <FiFileText className="stat-icon" style={{ color: 'var(--info)' }} />
              <div>
          ...
          <div className="space-y-3">
            {analytics.popularCourses.map((course: any, idx: number) => (
              <div key={idx} className="popular-course-item">
                <div>
                  <h4>{course.title}</h4>
                  <p className="text-sm text-tertiary">{course.enrollments} enrollments</p>
                </div>
                <div className="progress-bar" style={{ width: '200px' }}>
                  <div 
                    className="progress-fill" 
                    style={{ width: `${(course.enrollments / analytics.totalEnrollments) * 100}%` }}
                  ></div>
                </div>
              </div>
            ))}
