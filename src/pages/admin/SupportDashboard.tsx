...
import { FiSearch, FiSend, FiUser, FiClock, FiCheck } from 'react-icons/fi';
...
const SupportDashboard: React.FC = () => {
    ...
    if (user) {
      ...
      newSocket.on('message_sent', (message: Message) => {
        ...
        if (selectedUser && message.receiver === selectedUser._id) {
          ...
          setMessages(prev => {
            const exists = prev.some(m => m.message === message.message && 
  ...
  const fetchActiveChats = async () => {
    try {
      const response = await axios.get('/api/chat/recent', { withCredentials: true });
  ...
  const fetchChatHistory = async (userId: string) => {
    try {
      const response = await axios.get(`/api/chat/history/${userId}`, { withCredentials: true });
  ...
  const markAsRead = async (userId: string) => {
    try {
      await axios.put(`/api/chat/read/${userId}`, {}, { withCredentials: true });
        ...
        <div className="chat-list">
          {filteredChats.length === 0 ? (
            ...
            filteredChats.map(chat => (
              <div 
                key={chat._id} 
                className={`chat-item ${selectedUser?._id === chat._id ? 'active' : ''}`}
                    ...
                    <span className="bubble-time">
                      ...
                      {msg.sender === user?._id && (
                        <span className="read-status">
                          {msg.isRead ? <FiCheck className="read" /> : <FiCheck />}
                        </span>
                      )}
                    </span>
                  </div>
